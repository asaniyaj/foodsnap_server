...
@Path("/nutritionclass_name")
public class GetNutritionalInformation_name {
	
	private static final Logger logger = Logger.getLogger(GetNutritionalInformation_name.class);
	
	@GET
	@Produces(MediaType.TEXT_PLAIN)
	public String getNutrition_name(@DefaultValue("Plain French Toast") @QueryParam("foodname") String foodname ,
			@DefaultValue("100") @QueryParam("height") String height,
			@DefaultValue("100") @QueryParam("weight") String weight, 
			@DefaultValue("1") @QueryParam("version") int version) {

		if (logger.isDebugEnabled()) {
			logger.debug("Start getNutrition_name");
            ...
            switch(version){
	            case 1:
	                ...
	                try {
	        			...
	        			l = ProductInformation.getProductInfo_Model("food_name", foodname, cal);
