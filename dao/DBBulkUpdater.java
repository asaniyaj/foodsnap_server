...
public class DBBulkUpdater {
	...
	public void insertProductInfo_DB(List<NutritionalInfo> iList) throws Exception {
		Statement stmt = dbconn.createStatement();

		String iString = createInsertString(iList);
	...
	public void insertBarcodeFoodIdMapping_DB(List<BarcodeFoodIdMapping> iList_mapping) throws Exception {
		Statement stmt = dbconn.createStatement();

		String iString = createInsertString_Mapping(iList_mapping);
	...
	private String createInsertString_Mapping(List<BarcodeFoodIdMapping> iList) {
		String retStr = "";
		for (BarcodeFoodIdMapping m : iList) {
			retStr = retStr + "('" + m.getBarcode() + "', " + m.getFood_id() + "), ";
		}
		return retStr.substring(0, retStr.length() - 2);
	}

	private String createInsertString(List<NutritionalInfo> iList) {
		String retStr = "";
		for (NutritionalInfo ni : iList) {
